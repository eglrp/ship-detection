...
namespace caffe {
	...
	*/
	...
	inline __device__ float cubic_coeff_gpu(float x) {
		x = (x>0) ? x : -x;
		if (x<1){
	...
	protected:
		...
		float cubic_coeff(float x);

		float ResampleCubic(float x, float y, const Dtype * pdfValue, int nWidth, int nHeight, int pool_index, int* argmax_data, Dtype* w_data);
