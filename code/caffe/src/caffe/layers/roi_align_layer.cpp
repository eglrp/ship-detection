...
namespace caffe {
	...
	float ROIAlignLayer<Dtype>::cubic_coeff(float x){
		x = (x>0) ? x : -x;
		if (x<1){
	...
	template <typename Dtype>
	float ROIAlignLayer<Dtype>::ResampleCubic(float x, float y, const Dtype * pdfValue, int nWidth, int nHeight, int pool_index, int* argmax_data, Dtype* w_data)
	{
		Dtype dfCubicValue;
		int i = x;
		int j = y;
	...
	void ROIAlignLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		ROIALIGNPoolingParameter roi_align_param = this->layer_param_.roi_align_param();
		...
		pad_ratio_ = roi_align_param.pad_ratio();
		bi_type = roi_align_param.bi_type();
		is_multi_interpolate = roi_align_param.is_multi_interpolate();
