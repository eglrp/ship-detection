...
void ReshapeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "bottom[0]height " << bottom[0]->num();
  LOG(INFO) << "bottom[0]width " << bottom[0]->channels();
  LOG(INFO) << "bottom[0]height " << bottom[0]->height();
  LOG(INFO) << "bottom[0]width " << bottom[0]->width();
...
//newly edited by us
  if ( inferred_axis_ == -2 ){
    ...
    vector<int> top_shape_fix = bottom[0]->shape();
    top_shape_fix.resize(2);
    top_shape_fix[0] = bottom[0]->num();
    top_shape_fix[1] = bottom[0]->channels();
    top[0]->Reshape(top_shape_fix);
